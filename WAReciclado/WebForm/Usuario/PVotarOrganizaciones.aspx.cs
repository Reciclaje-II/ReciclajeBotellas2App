...
public partial class PVotarOrganizaciones : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            eCampaniaOrganizacion = swlnReciclado.Obtener_RCampaniaOrganizacion_O_Campania(eCampania.Nombre).ToList();
            lblCampaña.Text = eCampania.Nombre;
        }
        else
        ...
        if (eCampaniaOrganizacion != null)
        {
            foreach (var item in eCampaniaOrganizacion)
            {
                ...
                string pathOrganizacion = item.Organizacion;
                pathOrganizacion = pathOrganizacion.ToLower();
                pathOrganizacion = Regex.Replace(pathOrganizacion, @"\s", "");
                cardOrganizacion.pathImage = @"/Imagenes/Logo/" + pathOrganizacion + ".png";
                int total = swlnReciclado.Obtener_RVotos_O_Campania(item.Campania);
                int votos = swlnReciclado.Obtener_RVotos_O_Campania_Organizacion(item.Campania, item.Organizacion);
                ...
                cardOrganizacion.campaniaNombre = item.Campania;
                cardOrganizacion.organizacionNombre = item.Organizacion;
